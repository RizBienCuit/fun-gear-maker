...
if yoyo=="y":
    print("this program will make the fitting gear with the one you put in with any ratio.\n It will output an OpenSCAD script that will have to be rendered in OpenSCAD to get the svg of the new gear.\n The svg file that contains your original gear has to be composed with only one svg path without transform and without curves (only segments).\n the svg path has to be star shaped about the 0,0 point. \n !!! be careful: in inkscape, the 0,0 point is the up left corner of the page!!!")
...
nFichierOutOpenSCAD = input("new OpenSCAD script path: ")
fichierOutOpenSCAD = open(nFichierOutOpenSCAD, "w")
textOutOpenSCAD = "module Maitre(){\n   translate([0,-297,0])\n  import(\"" + nFichierIn + "\");\n}"
...
ratio=float(input("ratio\n(number of rotations that the other gear makes when the original makes one)\n!!!the step number (precision) must be divisible by the ratio\n more than one is not counciled if your original gear is not regular: "))
...
if teethState=="y":
    print("the program will output a new svg file wich contains your original gear with teeth added")
   nFichierInTeeth=input("path of this nex gear:")
...
while compta<NPas:
    ...
    textOutOpenSCAD += "\nrotate([0,0," + str(rototo) + "])\ntranslate([" + str(-D) + ",0,0])" + "\nrotate([0,0," + str(compta*stepLenght/ratio) + "])\nMaitre();"
    compta+=1
...
textOutOpenSCAD+="}"+"}"
fichierOutOpenSCAD.write(textOutOpenSCAD)
print(textOutOpenSCAD)
fichierOutOpenSCAD.close()
print(rototo)
