...
nFichierIn=input("original gear path: ")#"/home/ruben/Documents/svgscritp/troisieme.svg"
fichierIn = open(nFichierIn, "r")
textIn=fichierIn.read()
nFichierOut=input("new gear path: ")#"/home/ruben/Documents/svgscritp/troisiemeMAKE.svg"
...
def deterInter(indexSegment,indexDeusopoint,rayon):
    #indexS:l'index du premier point du segment dans linelist, indexD:idem,mais 2em point,pente du rayon, cote :si le rayon part a droite(1) ou a gauche, ray :0-rayon pas verticos 1-rayon verticos vers le haut -1-rayon verticos vers le bas
    cote=1
    rayonVertical=0
    penteRayon=0
    if rayon==90:
...
print(equaSeg)
print(paraSeg)
stepLenght=360/NPas#la longueur(degrés) de chaques pas
leSegmentCoupant=-1
compta=0
...
textOut=textIn
indexString=textOut.index("path")
indexString+=textOut[indexString:].index(" d=")
indexString+=textOut[indexString:].index("\"")+1
indexRetenu=indexString
indexString+=textOut[indexString:].index("\"")
textOut=textOut[:indexRetenu]+textOut[indexString:]
#print(textOut)
textEntre=" M "
compta=0
...
textOut=textOut[:indexRetenu]+textEntre+"z"+textOut[indexRetenu:]
